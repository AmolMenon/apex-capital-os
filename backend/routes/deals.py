from typing import Any, List
from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.orm import Session
from db.database import get_db
from db.models import RoleEnum, User
from repositories.deal_repository import deal_repo
from schemas.deal import DealCreate, DealUpdate, DealResponse
from auth.dependencies import get_current_active_user, require_roles

router = APIRouter()

@router.get("/", response_model=List[DealResponse])
def read_deals(
    db: Session = Depends(get_db),
    skip: int = 0,
    limit: int = 100,
    current_user: User = Depends(get_current_active_user)
) -> Any:
    return deal_repo.get_multi(db, skip=skip, limit=limit)

@router.post("/", response_model=DealResponse)
def create_deal(
    *,
    db: Session = Depends(get_db),
    obj_in: DealCreate,
    current_user: User = Depends(require_roles([RoleEnum.ADMIN, RoleEnum.PARTNER, RoleEnum.PRINCIPAL, RoleEnum.ASSOCIATE]))
) -> Any:
    return deal_repo.create(db, obj_in=obj_in)

@router.get("/{id}", response_model=DealResponse)
def read_deal(
    *,
    db: Session = Depends(get_db),
    id: int,
    current_user: User = Depends(get_current_active_user)
) -> Any:
    obj = deal_repo.get(db=db, id=id)
    if not obj:
        raise HTTPException(status_code=404, detail="Deal not found")
    return obj

@router.put("/{id}", response_model=DealResponse)
def update_deal(
    *,
    db: Session = Depends(get_db),
    id: int,
    obj_in: DealUpdate,
    current_user: User = Depends(require_roles([RoleEnum.ADMIN, RoleEnum.PARTNER, RoleEnum.PRINCIPAL, RoleEnum.ASSOCIATE]))
) -> Any:
    obj = deal_repo.get(db=db, id=id)
    if not obj:
        raise HTTPException(status_code=404, detail="Deal not found")
    obj = deal_repo.update(db=db, db_obj=obj, obj_in=obj_in)
    return obj

@router.delete("/{id}", response_model=DealResponse)
def delete_deal(
    *,
    db: Session = Depends(get_db),
    id: int,
    current_user: User = Depends(require_roles([RoleEnum.ADMIN, RoleEnum.PARTNER]))
) -> Any:
    obj = deal_repo.get(db=db, id=id)
    if not obj:
        raise HTTPException(status_code=404, detail="Deal not found")
    obj = deal_repo.remove(db=db, id=id)
    return obj
