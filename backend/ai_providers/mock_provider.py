...
class MockProvider(BaseAIProvider):
    ...
    def generate_structured_output(self, task_type: str, prompt: str, schema_name: str) -> Optional[Dict[str, Any]]:
        ...
        if task_type == "investment_memo":
            if is_supertails(prompt): return _supertails_memo()
            return self._mock_memo(prompt)
            
        if task_type == "market_research":
            if is_supertails(prompt): return _supertails_market_map()
            return self._mock_market_map()
            
        if task_type == "competitor_research":
            if is_supertails(prompt): return _supertails_competitors()
            return self._mock_competitor()
            
        if task_type == "diligence_plan":
            if is_supertails(prompt): return _supertails_diligence()
        ...
        if task_type == "ic_recommendation":
            if is_supertails(prompt): return _supertails_ic_recommendation()
            return self._mock_ic_recommendation()
            
        if task_type == "partner_pushback":
            if is_supertails(prompt): return _supertails_partner_pushback()
    ...
    def _mock_memo(self, prompt: str):
        deal_title, profile = self._get_deal_profile(prompt)
        sector = profile.get("sector", "Enterprise Software")
        desc = profile.get("description", "a stagnant vertical")
        
        return {
            "executive_summary": (
                f"{deal_title} presents a highly differentiated, generational investment opportunity within the deeply fragmented {sector} space. "
                f"Unlike legacy incumbents that rely on bloatware and massive consulting fees, {deal_title} is exploiting a structural weakness in the market by deploying a hyper-efficient, highly automated wedge. "
                f"Our diligence confirms that {desc}. The founding team combines an exceptional pedigree of deep operational expertise and world-class technical talent, allowing them to iterate faster than any known competitor. "
                "Early traction metrics indicate a profoundly sticky product; we are seeing best-in-class Net Revenue Retention (NRR) and a dramatically compressed CAC payback period, which proves their go-to-market motion is scalable and mathematically sound. "
                "While the space is undeniably competitive, the sheer velocity of their execution and the compounding nature of their data network effects give them a distinct trajectory toward monopolistic market share. "
                "At the current entry valuation, the asymmetric upside heavily outweighs the execution risk, making this a high-conviction 'Strong Invest'."
            ),
            "problem": (
                f"The target market in {sector} is severely bogged down by legacy technical debt. Current operational workflows require heavy manual intervention, resulting in unacceptable error rates and massive labor costs. "
                ...
                "Furthermore, the buyers are increasingly frustrated with opaque pricing models and multi-year lock-ins for software that fails to deliver actual ROI. "
                "This creates a massive 'data silo' problem, preventing leadership from extracting actionable intelligence from their operations and stunting overall industry growth."
            ),
            "solution": (
                f"{deal_title} deploys a highly elegant, deeply integrated SaaS ecosystem that completely automates these friction points. "
                "By acting as the central nervous system for the workflow, it eliminates manual data entry and provides real-time, AI-driven insights directly to the decision-makers. "
                "The platform's consumer-grade UI ensures massive internal adoption from day one, while its enterprise-grade security architecture (SOC2 Type II, ISO 27001) satisfies the most stringent compliance requirements of Fortune 500 CISOs. "
                "Crucially, the architecture is API-first, allowing it to seamlessly hook into existing infrastructure rather than requiring a rip-and-replace, radically accelerating time-to-value."
            ),
            "market_opportunity": (
                ...
                f"By capturing the highly lucrative mid-market and enterprise tiers in the {sector} space, {deal_title} is targeting a multi-billion dollar Serviceable Addressable Market (SAM). "
                ...
                "Furthermore, there are significant expansion opportunities into adjacent verticals and international markets once the core domestic playbook is fully parameterized."
            ),
            "founder_market_fit": (
                f"The founding team possesses a terrifying level of operational intensity and deep empathy for the problem space. "
                "Having previously spent years scaling infrastructure at leading incumbents, they understand the exact technical debt and organizational inertia of the industry. "
                "They have successfully attracted a cartel of elite engineering talent, proving their ability to sell the vision. This is the exact archetype of a team that wins: highly analytical, relentlessly resourceful, and completely obsessed with the customer."
            )
        }
    ...
    def _mock_ic_recommendation(self):
        return {
            "decision": "Invest",
            "explanation": "The team is exceptional, the wedge is sharp, and the unit economics indicate highly efficient, capital-light growth. The primary concern is scaling the enterprise go-to-market motion against entrenched incumbents, which we condition upon 3 positive reference calls with enterprise buyers and a deep-dive into the LTV/CAC ratio of the latest cohorts.",
            "key_catalysts": ["Reaching $1M ARR threshold", "Signing a top 10 Fortune 500 logo", "Successful rollout of the v2 AI infrastructure"]
        }
