...
def test_quota_missing():
    ...
    result = subprocess.run([sys.executable, "evaluation/run_frozen_pilot.py", "--case", "VC_01", "--experiment-batch", "test"], env=env, capture_output=True, text=True, cwd="backend")
...
def test_quota_sufficient():
    ...
    result = subprocess.run([sys.executable, "evaluation/run_frozen_pilot.py", "--case", "VC_01", "--experiment-batch", "test", "--preflight-only"], env=env, capture_output=True, text=True, cwd="backend")
