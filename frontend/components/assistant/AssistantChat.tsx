...
export function AssistantChat({ dealId, crossDeal = false, fullHeight = false }: { dealId?: string | number, crossDeal?: boolean, fullHeight?: boolean }) {
  ...
  const loadSession = async () => {
    try {
      const res = await api.getAssistantSession(dealId as string);
      if (res && res.messages) {
  ...
  const loadSuggested = async () => {
    try {
      const res = await api.getSuggestedAssistantQuestions(dealId as string);
  ...
  const clearSession = async () => {
    if (dealId && !crossDeal) {
      await api.clearAssistantSession(dealId);
      setMessages([]);
    }
  };
  ...
  const askQuestion = async (q: string) => {
    ...
    try {
      let answer;
      if (crossDeal) {
         answer = await api.askWorkspaceAssistant(q);
      } else {
         answer = await api.askDealAssistant(dealId as string, q);
      }
  ...
  const renderAssistantAnswer = (msg: any) => {
    if (!msg.fullAnswer) {
      ...
      <div className="flex items-center justify-between p-4 bg-background/60 backdrop-blur-md border-b border-emerald-500/20 shadow-sm relative overflow-hidden">
        <div className="flex items-center relative z-10">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground font-medium tracking-wider uppercase">Analysis Engine</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-8 relative">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center px-4 relative z-10">
            ...
            <h4 className="text-xl font-bold text-foreground mb-3 tracking-tight">AI Assistant Ready</h4>
            <p className="text-sm text-muted-foreground max-w-sm mb-8 leading-relaxed">
              {crossDeal 
                ? "I am connected to your entire portfolio and pipeline. Ask me to find deals, run comparisons, or generate market insights."
            ...
            <div className="w-full max-w-md space-y-3">
              {(!crossDeal && suggested.length > 0 ? suggested.slice(0, 4) : crossDeal ? [
                "Find AI startups under $10M ARR",
                "Compare NeuralDesk with Rippling",
                "What concerns should I investigate for Sarvam?",
