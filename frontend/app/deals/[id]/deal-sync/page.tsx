...
import { InvestmentThesisPanel } from "@/components/deal-sync/InvestmentThesisPanel"
import { AntiThesisPanel } from "@/components/deal-sync/AntiThesisPanel"
import { WhatMustBeTrueTable } from "@/components/deal-sync/WhatMustBeTrueTable"
import { ConvictionDashboard } from "@/components/deal-sync/ConvictionDashboard"
import { PartnerPersonaGrid } from "@/components/deal-sync/PartnerPersonaGrid"
import { ICSimulationPanel } from "@/components/deal-sync/ICSimulationPanel"
import { FundMathPanel } from "@/components/deal-sync/FundMathPanel"
import { ChangeOurMindPanel } from "@/components/deal-sync/ChangeOurMindPanel"
...
export default function WarRoomPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        console.warn("Deal Sync data not found yet.")
      } finally {
        setLoading(false)
      }
    }
  ...
  const runWarRoom = async () => {
    ...
    try {
      ...
      console.error(error)
      alert("Failed to run Deal Sync Engine.")
    } finally {
      setRunning(false)
    }
  }
  ...
  if (loading) {
    return <div className="p-12 text-center text-muted-foreground"><Loader2 className="h-8 w-8 animate-spin mx-auto mb-4" /> Loading Deal Sync...</div>
  }
    ...
    <div className="container py-8 max-w-7xl mx-auto space-y-6 px-4 md:px-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <Badge variant="outline" className="uppercase tracking-widest text-[10px]">Deal Sync</Badge>
          </div>
          <p className="text-muted-foreground max-w-2xl">
            Autonomous Deal Deal Sync & IC Simulator. Translating evidence into investment conviction, partner debate, and fund math.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Button onClick={runWarRoom} disabled={running}>
            ...
            {warRoom ? "Re-Run Deal Sync" : "Run Deal Sync"}
          </Button>
        </div>
      </div>
      ...
      {warRoom && (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
          ...
          <div className="bg-muted/30 p-4 rounded-lg border">
            <div className="text-xs font-semibold text-muted-foreground uppercase mb-1">Deal Sync Status</div>
      ...
      {warRoom && (
        <div className="bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-lg p-5">
          ...
          <p className="text-sm text-blue-800/80 dark:text-blue-300/80 mb-4">Click any question to ask the Analyst Assistant using the Deal Sync's generated context.</p>
          <div className="flex flex-wrap gap-2">
            {[
              ...
              <Link key={q} href={`/deals/${id}/assistant`}>
      ...
      {!warRoom ? (
        <div className="py-24 text-center border rounded-lg bg-muted/10 border-dashed">
          ...
          <h3 className="text-lg font-bold mb-2">Deal Deal Sync is Empty</h3>
          <p className="text-muted-foreground max-w-md mx-auto mb-6">
            Run the Autonomous Deal Deal Sync to simulate an investment committee debate, calculate conviction, and model fund returns.
          </p>
          <Button size="lg" onClick={runWarRoom} disabled={running}>
            {running ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <PlayCircle className="mr-2 h-4 w-4" />}
            Start Deal Sync Simulation
          </Button>
        </div>
      ) : (
